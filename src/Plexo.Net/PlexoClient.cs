...
namespace Plexo
{
    public class PlexoClient : IPlexoClient
    {
        ...
        public async Task<ServerResponse<SplittedTransaction>> SplitPurchaseAsync(SplitPaymentRequest payment)
        {
            // Sign request
            var signedClientRequest = SignClientRequest(payment);
            ...
            ServerSignedResponse<SplittedTransaction> signedServerResponse;

            // Post async signed request as ByteArrayContent
            using (var request = new HttpRequestMessage(HttpMethod.Post, "Operation/SplitPurchase"))
            {
                request.Content = byteContent;
                ...
                using (var jsonReader = new JsonTextReader(streamReader))
                {
                    ...
                    // Read the response content from a stream
                    signedServerResponse = serializer.Deserialize<ServerSignedResponse<SplittedTransaction>>(jsonReader);
                }
            }
